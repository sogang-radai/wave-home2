#!/usr/bin/env bash
# 데모 프로필(wave-server :8502)에 맞게 에이전트 .env 의 internal API 설정을 갱신한다.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
AGENT_DIR="$ROOT/wave-home-agent"
ENV_FILE="$AGENT_DIR/.env"
DEMO_INTERNAL_URL="${DEMO_INTERNAL_URL:-http://127.0.0.1:8502/internal/v1}"
DEMO_CORE_URL="${DEMO_CORE_URL:-http://127.0.0.1:8502}"
