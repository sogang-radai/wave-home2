"""
Preprocess PCR files (dataset/set0) for sleep-net0-2 (5 s bed window).

Same as preprocess_set0-0.py except:
  BED_WINDOW = 100  (~5 s @ 20 fps)
  BED_STRIDE scaled proportionally (×100/160)

Output: sleep-net/samples/set0-1.pkl
"""
...
OUTPUT_PKL = SAMPLES_DIR / "set0-1.pkl"
...
# Scaled from set0-0 strides (×100/160).
BED_STRIDE: Dict[int, int] = {0: 25, 1: 20, 2: 12, 3: 12, 4: 12}
...
def make_windows(region_start: int, region_end: int, window: int, stride: int) -> List[List[int]]:
...
def build_dataset() -> dict:
    ...
    log(f"bed_window={BED_WINDOW} toss_window={TOSS_WINDOW}")
    log()
    ...
    meta = {
        ...
        "preprocess_script": "preprocess_set0-1.py",
        "normalization": {
            "x": [X_MIN, X_MAX],
            "y": [Y_MIN, Y_MAX],
            "z": [Z_MIN, Z_MAX],
...
def main() -> None:
    log("=" * 70)
    log("Preprocessing set0 for sleep-net0-2 (bed_window=100)")
    log("=" * 70)
