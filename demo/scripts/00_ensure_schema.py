#!/usr/bin/env python3
"""기존 bin/data/demo.db 를 데이터 유지한 채 스키마만 보강한다.
...
실행:
    uv run --with sqlite-vec demo/scripts/00_ensure_schema.py
    # 또는
    python3 demo/scripts/00_ensure_schema.py
"""
...
def main() -> None:
    if not DB_PATH.exists():
        raise SystemExit(f"demo.db 없음: {DB_PATH}\n먼저 01_gen_raw_data.py 를 실행하세요.")
